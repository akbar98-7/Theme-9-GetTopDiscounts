...
import { apiCompanyUpdatedData, apiGetMetaData, apiGetWidgets, apiTemplate } from '@/apis/user';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const googleVerificationContent = headerWidgets.find(
    (widget) => widget.google_site_verification_content
...
export default async function RootLayout({
  ...
  if (pathname?.includes('/out/O/')) {
    return (
      <html lang="en">
        <body style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#f8f9fa', color: '#333', textAlign: 'center' }}>
          <div>
  ...
  if (!companyData || companyData.website_is_active == 0) {
    return (
      <html lang="en">
        <body style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: '#f8f9fa', color: '#333', textAlign: 'center' }}>
          <div>
            <h1 style={{ fontSize: '3rem', marginBottom: '1rem' }}>This site can’t be reached</h1>
            <p style={{ fontSize: '1.2rem', marginBottom: '1rem' }}>We’re sorry, but the site you’re looking for is currently unavailable.</p>
          </div>
        </body>
      </html>
    );
    ...
    return (
      <html lang="en">
        <head>
          ...
          <link rel="preconnect" href={`https://${companyDomain.domain}`} crossOrigin="anonymous" />
          <HeaderScripts scripts={headerWidgets} />
          {companyDomain.domain === 'gettopdiscounts.com' && (
            <>
              <meta name="fo-verify" content="79f1bdb3-e89f-4471-b437-ff23bf7bf436" />
              <meta name='impact-site-verification' content='2d8da9ff-b354-490f-835c-b8aa6118ac42' />
              <meta name="p:domain_verify" content="4742214395778362dc116576b80400f1"/>
              <meta name="verification" content="7b6f47eae6f1418085035a437fe15c84" />
              <meta name="verification" content="d8489cd0be98463b984368350d8745cd" />
            </>
          )}
          {companyDomain.domain === 'verifiedlivecoupons.com' && (
            <>
              <meta name="verification" content="f670540aeb758bc6bbc7ee52295d6d99" />
            </>
          )}
        </head>
        <body>
          {companyDomain.domain === 'gettopdiscounts.com' && (
            <noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-PD85GTM2"
            height="0" width="0" style={{display:'none',visibility:'hidden'}}></iframe></noscript>
          )}
          {companyDomain.domain === 'verifiedlivecoupons.com' && (
            <noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TDGSDPCC"
            height="0" width="0" style={{display:'none',visibility:'hidden'}}></iframe></noscript>
          )}
          <BodyTopScripts scripts={bodyTopWidgets} />

          <ThemeSwitcher template={template}>
            {children}
          </ThemeSwitcher>

          <BodyBottomScripts scripts={bodyBottomWidgets} />
          <OrganizationSchema domain={companyDomain?.domain} socialLinks={socialLinks} companyLogo={companyData?.company_logo} companyName={companyData?.company_name} legalName={companyData?.company_legal_name} alternateName={companyData?.company_alternate_name}/>
        </body>
      </html>
    );
  }
