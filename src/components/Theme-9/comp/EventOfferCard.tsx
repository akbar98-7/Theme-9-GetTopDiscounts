...
const EventOfferCard = ({ product, merchantHref, domain, merchant_name, merchant_logo }: any) => {
    ...
    return (
        <div className="group relative bg-white rounded-[2.5rem] border border-[#2ECC71] flex flex-col h-full transition-all duration-500 hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.12)] hover:-translate-y-2 overflow-hidden">
            
            {/* --- Top Floating Badges --- */}
            <div className="absolute top-4 left-4 right-4 z-20 flex justify-between items-start pointer-events-none">
                {finalDiscountTag && (
                    <div className="bg-[#2ECC71] text-white text-[10px] font-black px-3.5 py-1.5 rounded-full shadow-lg shadow-rose-200 uppercase tracking-widest flex items-center gap-1.5 animate-pulse-slow">
                ...
                {product?.coupon_code && (
                    <div className="bg-[#2ECC71] text-white p-2 rounded-xl shadow-lg shadow-indigo-100">
                        <Tag size={14} />
                    </div>
                )}
            </div>
            ...
            <div className="px-6 pb-6 pt-2 flex flex-col flex-grow">
                ...
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <Link href={merchantHref} className="text-[11px] font-bold text-slate-400 hover:text-[#2ECC71] uppercase tracking-widest transition-colors">
                        {merchant_name}
                    </Link>
                </div>
    ...
    {product?.coupon_code ? (
        ...
        <OfferOutUrl
            ...
            merchantHref={merchantHref}
            domain={domain}
            customClass="no-underline w-full bg-[#2ECC71] !text-white py-4 rounded-[1.25rem] font-bold text-[11px] uppercase tracking-[0.15em] flex items-center justify-center gap-2 hover:bg-slate-900 transition-all duration-300 shadow-xl shadow-indigo-100"
        >
            ...
            {/* Bottom Accent Line */}
            <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-[#2ECC71] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
        </div>
    );
}
