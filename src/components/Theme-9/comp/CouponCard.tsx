...
const CouponCard = async ({ product, merchantHref, domain, merchant_name, merchant_logo, pageType }: any) => {
    ...
    return (
        <div className="group relative bg-[#1E293B]/50 backdrop-blur-sm border border-slate-700/50 rounded-[2.5rem] p-8 flex flex-col h-full transition-all duration-500 hover:border-[#2ECC71]/50 hover:shadow-[0_20px_50px_rgba(46,204,113,0.15)] hover:-translate-y-2">
            {/* Discount Badge */}
            {finalDiscountTag && (
                <div className="absolute -top-3 -right-3 bg-gradient-to-br from-[#2ECC71] to-[#2ECC71] text-white text-[11px] font-black px-5 py-2 rounded-2xl shadow-xl shadow-blue-900/20 z-20 flex items-center gap-2">
            ...
            <div className="flex-grow">
                ...
                <div className="flex justify-between items-start mb-8">
                    ...
                    <div className="flex flex-col items-end">
                        <div className="flex items-center gap-1.5 text-[#2ECC71] font-bold text-[10px] uppercase tracking-widest bg-blue-500/10 px-3 py-1.5 rounded-full mb-2">
                ...
                <div className="space-y-3">
                    <h4 className="text-xl font-bold text-white leading-tight group-hover:text-[#2ECC71] transition-colors line-clamp-2 min-h-[56px]">
            ...
            <div className="mt-10 space-y-4">
                <OfferOutUrl
                    ...
                    customClass={`w-full relative h-14 flex items-center justify-center rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all duration-300 active:scale-95 overflow-hidden ${
                        ...
                        : "bg-white text-slate-900 hover:bg-[#2ECC71] hover:text-black no-underline"
                    }`}
                >
                    {product?.coupon_code ? (
                        <div className="flex items-center gap-3">
                            <span className="bg-[#2ECC71] px-3 py-1 rounded-lg">
                ...
                {pageType !== 'events' && (
                    <Link href={merchantHref} className="no-underline text-white block text-center group/link">
                        <span className="text-[12px] font-bold text-slate-400 uppercase tracking-widest group-hover/link:text-[#2ECC71] transition-colors">
