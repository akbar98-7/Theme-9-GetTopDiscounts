...
import P9 from '@theme9/Pages/All-Products/[...slug]/page'
...
if (slug.length === 3) {
    ...
    const formattedData = JSON.stringify(["specific-product", removeTrailingIdFromSlug(slug[2])]);
    ...
    if (productId) {
        ...
        const formattedData = JSON.stringify(["specific-product", removeTrailingIdFromSlug(slug[1])]);
        meta = (await apiGetMetaData(formattedData, companyDomain.domain)).data;
    } else {
        // /products/merchant/category
        const formattedData = JSON.stringify(["brand-category-product-page", slug[1], slug[0]]);
        meta = (await apiGetMetaData(formattedData, companyDomain.domain)).data;
