...
const ProductCard = async ({ offer, mer_slug, mer_slug_type, type }: any) => {
  ...
  return (
    <div className="group relative w-[260px] min-h-[420px] flex-shrink-0 
      ...
      hover:-translate-y-2 hover:border-[#2ECC71]/40 
      shadow-[0_20px_60px_-15px_rgba(0,0,0,0.8)] hover:shadow-[#2ECC71]/10">
      ...
      <div className="relative h-44 rounded-2xl bg-white overflow-hidden flex items-center justify-center">
        ...
        {/* DISCOUNT TAG - CHANGED TO GREEN */}
        {finalDiscountTag && (
          <div className="absolute top-3 right-3 flex items-center gap-1 
            rounded-full bg-black/80 px-3 py-1 text-[10px] 
            font-black text-[#2ECC71] border border-[#2ECC71]/30">
            <Flame size={12} className="fill-[#2ECC71]" />
            {finalDiscountTag}
          </div>
        )}
      </div>
      ...
      <div className="mt-4 space-y-3">
        ...
        {/* TITLE - HOVER COLOR CHANGED TO GREEN */}
        <h3 className="text-sm font-bold text-white leading-snug line-clamp-2 min-h-[40px] group-hover:text-[#2ECC71] transition">
          {type === "product"
            ? product?.offer_title
      ...
      {/* GET DEAL BUTTON - FULL GREEN THEME (NO BLUE GRADIENT) */}
      <div className="mt-auto pt-6">
        <OfferOutUrl
          ...
          merchantHref={merchantHref}
          domain={domain}
          customClass="no-underline block w-full rounded-xl bg-[#27AE60] 
          py-3 text-center text-xs font-black uppercase tracking-[0.15em] text-white 
          hover:bg-[#219150] transition-all 
          active:scale-95 shadow-lg shadow-green-900/20"
        >
          Get Deal
        </OfferOutUrl>
      </div>
