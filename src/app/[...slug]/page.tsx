...
export async function generateMetadata({
  ...
  const meta = (await apiGetMetaData(JSON.stringify(slug), companyDomain.domain)).data;
  ...
  // } 
    ...
    ogImage = meta?.og_image ? getBaseImageUrl(companyDomain.domain, meta?.og_image, companyLogo) : companyLogo;
  } 
  else if (slug.length === 2 && slug[0] === meta?.company_data?.promotion_slug) {
    canonicalData += `/${slug[0]}/${slug[1]}`;
    ogImage = meta?.og_image ? getBaseImageUrl(companyDomain.domain, meta?.og_image, companyLogo) : companyLogo;
    if (slug[1] === 'christmas-holiday-deals'){
      ogImage = getBaseImageUrl(companyDomain.domain, "/shared-assets/promotion/christmas-holiday-deals.png", '');
    }
  } 
  ...
  return {
    ...
    openGraph: {
      ...
      description: discardHTMLTags(meta?.twitter_description) ?? discardHTMLTags(meta?.meta_description),
      images: [
        {
          url: ogImage,
...
const Dynamic = async ({
  ...
  return (
    <SelectedPage params={params} searchParams={searchParams} />
  )
